...
int main()
{
    int digit1,digit2,digit3,digit4,num,i,temp,orginal;
    printf("Enter the four digit encrypted integer: ");
    scanf("%d",&num);
    while (!(num>=1000&&num<=9999)){
            printf("Invalid number. Enter a four digit encrypted number: ");
            scanf("%d",&num);
    }
    temp=num;
    digit1=temp%10;
    ...
    digit2=digit4;
    digit4=temp;
    orginal= digit1+digit2*10+digit3*100+digit4*1000;
    printf("Orginal integer is %d",orginal);
}
// Homework for the Introduction to Computer Programming lesson
//C: How to Program by Paul and Harvey Deitel chapter3 page96 question 3.49 second program
